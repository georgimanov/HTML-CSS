jQuery(function($) {
	...
	*/
	...
	jQuery('.why_us').each(function(){
		jQuery(this).find('.item_content').each(function(num){
			jQuery(this).prepend('<span class="dropcap">0'+(num+1)+'</span>')
		})
	})
	jQuery('.principles').each(function(){
		jQuery(this).find('.item_content').each(function(num){
			jQuery(this).prepend('<span class="dropcap">0'+(num+1)+'.</span>')
		})
	})
